...
class Graph:
    ...
    def search_and_create_tree(
        ...
        for source_node_coord, ayirici_id, initial_conns in source_nodes:
            ...
            visited = set()
            visited_merkezler = {
                self.ayirici_coord_2_merkez_dict.get(source_node_coord)
            }
            ...
            while queue:
                ...
                if buffer_df is None:
                    ...
                    continue
                self.mark_visited_merkezler(buffer_df, visited_merkezler)
    ...
    def mark_visited_merkezler(self, buffer_df, visited_merkezler):
        if buffer_df is None or buffer_df.empty:
            return
        for merkez_id in buffer_df["target_merkez_id"]:
            if merkez_id is not None and not pd.isna(merkez_id):
                visited_merkezler.add(merkez_id)
    ...
    def process_paths_fast(
        self,
        source_node_coord,
        paths,
        baglanti_start,
        baglanti_end,
        visited_merkezler=None,
    ):
        """
        ...
        visited_merkezler = visited_merkezler or set()
        ...
        for _, path in paths.items():
            ...
            if target_merkez_id == source_merkez_id:
                continue
            if target_merkez_id in visited_merkezler:
                continue
            if self.has_intermediate_merkez_in_path(
                path, source_merkez_id, target_merkez_id
            ):
                self.hata_loglari.append(
                    {
                        ...
                        "bilgi": "path source/target disinda ara merkez ayiricisi iceriyor",
                    }
                )
                continue
    ...
    def has_intermediate_merkez_in_path(self, path, source_merkez_id, target_merkez_id):
        allowed_merkezler = {source_merkez_id, target_merkez_id}
        ayirici_2_merkez_dict = getattr(self, "ayirici_2_merkez_dict", {}) or {}
        for coord in path:
            ayirici_id = self.ayirici_coord_2_ayirici_id_dict.get(coord)
            if ayirici_id is None:
                continue
            merkez_id = ayirici_2_merkez_dict.get(
                ayirici_id, self.ayirici_coord_2_merkez_dict.get(coord)
            )
            if merkez_id not in allowed_merkezler:
                return True
        return False
    ...
    def trace_merkez_ici_1_bara(self, giris_ayirici, giris_hucre, giris_merkez, tablo):
        ...
        if self.hucre_2_coupling.get(giris_hucre, 0) == 1:
            return self.trace_merkez_ici_1_bara_from_coupling(
                giris_ayirici, giris_hucre, giris_merkez, tablo
            )
    ...
    def trace_merkez_ici_1_bara_from_coupling(
        self, giris_ayirici, giris_hucre, giris_merkez, tablo
    ):
        """Trace from a bus-coupling bay only toward the opposite bus section."""
        to_add_queue = []
        giris_coord = self.ayirici_id_2_coord.get(giris_ayirici)
        coupling_ayiricilar = self.ensure_list(
            self.hucre_2_ayiricilar_abb_id_dict.get(giris_hucre, [])
        )
        opposite_ayiricilar = [
            ayirici
            for ayirici in coupling_ayiricilar
            if ayirici is not None and ayirici != giris_ayirici
        ]

        if giris_coord is None or not opposite_ayiricilar:
            self.hata_loglari.append(
                {
                    "source_node": giris_ayirici,
                    "target_node": None,
                    "merkez": giris_merkez,
                    "bilgi": "tek bara kuplaj trace icin karsi ayirici bulunamadi",
                }
            )
            return []
        ...
        for opposite_ayirici in opposite_ayiricilar:
            if (
                self.shortest_path_in_merkez(
                    giris_ayirici, opposite_ayirici, giris_merkez
                )
                is None
            ):
                continue

            # First cross the coupling bay: D -> E in the user's diagram.
            self.ayirici_finder_in_path(
                giris_ayirici,
                opposite_ayirici,
                opposite_ayirici,
                giris_merkez,
            ...
            for hucre in cikis_hucreleri_list:
                ...
                for cikis_top_ayirici in cikis_top_ayiricilar:
                    target_ayirici = self.get_queue_ayirici_for_exit(
                        cikis_top_ayirici, cikis_bottom_ayirici
                    )
                    path = self.shortest_path_in_merkez(
                        opposite_ayirici, target_ayirici, giris_merkez
                    )
                    if path is None or giris_coord in path:
                        continue
    ...
    def get_queue_ayirici_for_exit(self, cikis_top_ayirici, cikis_bottom_ayirici):
        if cikis_top_ayirici == cikis_bottom_ayirici:
            return cikis_top_ayirici
        if cikis_bottom_ayirici is not None:
            return cikis_bottom_ayirici
        return None
    ...
    def ayirici_finder_in_path(
        ...
        if cikis_top_ayirici == cikis_bottom_ayirici:
            ...
            path = self.shortest_path_in_merkez(
                source_ayirici, cikis_top_ayirici, giris_merkez
            )
            ...
            path = self.shortest_path_in_merkez(
                source_ayirici, cikis_bottom_ayirici, giris_merkez
            )
        else:
            path = None
        ...
        if not self.path_stays_in_merkez(path, giris_merkez):
            self.hata_loglari.append(
                {
                    "source_node": source_ayirici,
                    "target_node": cikis_top_ayirici,
                    "merkez": giris_merkez,
                    "bilgi": "merkez ici trace path merkez disina cikiyor",
                }
            )
            return None
        ...
        for ayirici_1 in giris_top_ayiricilar:
            ...
            for cikis_hucresi in cikis_hucreleri_list:
                ...
                if ayirici_2 is not None:
                    coord_path = self.shortest_path_in_merkez(
                        ayirici_1, ayirici_2, merkez
                    )
                ...
                for baglama_hucresi in baglama_hucreleri:
                    ...
                    for baglama_ayirici_3 in baglama_ayiricilar:
                        ...
                        cp1 = self.shortest_path_in_merkez(
                            ayirici_1, baglama_ayirici_2, merkez
                        )
                        cp3 = self.shortest_path_in_merkez(
                            baglama_ayirici_3, ayirici_4, merkez
                        )
    ...
    def shortest_path_in_merkez(self, ayirici_id_1, ayirici_id_2, merkez):
        c1 = self.ayirici_id_2_coord.get(ayirici_id_1)
        c2 = self.ayirici_id_2_coord.get(ayirici_id_2)
        if c1 is None or c2 is None or c1 not in self.graph or c2 not in self.graph:
            return None
        if not self.is_coord_allowed_for_merkez(c1, merkez):
            return None
        if not self.is_coord_allowed_for_merkez(c2, merkez):
            return None

        cache_key = (merkez, c1, c2)
        if cache_key in self._merkez_path_cache:
            return self._merkez_path_cache[cache_key]
        ...
        while queue:
            node = queue.popleft()
            if node == c2:
                path = self.reconstruct_path(parent, c2)
                self._merkez_path_cache[cache_key] = path
                self._merkez_path_cache[(merkez, c2, c1)] = list(reversed(path))
                return path

            for neighbor in self.graph[node]:
                if neighbor in parent:
                    continue
                if not self.is_coord_allowed_for_merkez(neighbor, merkez):
                    continue
                parent[neighbor] = node
                queue.append(neighbor)

        self._merkez_path_cache[cache_key] = None
        self._merkez_path_cache[(merkez, c2, c1)] = None
        return None

    def is_coord_allowed_for_merkez(self, coord, merkez):
        ayirici_id = self.ayirici_coord_2_ayirici_id_dict.get(coord)
        if ayirici_id is None:
            return True
        return self.get_ayirici_merkez(ayirici_id, coord) == merkez

    def get_ayirici_merkez(self, ayirici_id, coord=None):
        ayirici_2_merkez_dict = getattr(self, "ayirici_2_merkez_dict", {}) or {}
        ayirici_coord_2_merkez_dict = (
            getattr(self, "ayirici_coord_2_merkez_dict", {}) or {}
        )
        merkez = ayirici_2_merkez_dict.get(ayirici_id)
        if merkez is None and coord is not None:
            merkez = ayirici_coord_2_merkez_dict.get(coord)
        return merkez
    ...
    def path_stays_in_merkez(self, path, merkez):
        if path is None:
            return False
        for coord in path:
            ayirici_id = self.ayirici_coord_2_ayirici_id_dict.get(coord)
            if ayirici_id is None:
                continue
            if self.get_ayirici_merkez(ayirici_id, coord) != merkez:
                return False
        return True
